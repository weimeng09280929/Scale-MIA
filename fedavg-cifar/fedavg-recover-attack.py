...
def attack(weight, bias, original_data, l1output, batch_size, idx):
    ...
    print(
        f"  [debug] batch_size={batch_size}, unique_bins_hit={unique_locs}, collision_rate={1 - unique_locs / batch_size:.3f}")
...
#    test_data = torchvision.datasets.FashionMNIST(root='./test', train=True, transform=ToTensor(), download=True)
    ...
    encoder.load_state_dict(
        torch.load(
            "models/encoder_model_" + str(aux) + ".pkl",
            map_location=device
        )
    )
    encoder.to(device)
    ...
    decoder.load_state_dict(
        torch.load(
            "models/decoder_model_" + str(aux) + ".pkl",
            map_location=device
        )
    )
    decoder.to(device)
