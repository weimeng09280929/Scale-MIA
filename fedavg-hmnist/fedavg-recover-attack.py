...
if __name__ == '__main__':
    ...
    decoder.load_state_dict(
        torch.load(
            "models/decoder_model_100_new.pkl",
            map_location=device
        )
    )
    decoder.to(device)
    ...
    bins = torch.load(
        "data/latent_small_bins_100_new.pt",
        map_location=device
    )
    ...
    for current_epoch in range(all_epoch):
        for idx in range(int(1024 / batch_size)):
            ...
            for j in range(client_num):
                ...
                aggregated_bias = aggregated_bias + model.fc1.bias.data.cpu().view(1024, 1).expand(1024,
                                                                                                   64 * 16) / client_num
                torch.cuda.empty_cache()
            estimated_weight_gradient = -(
                        aggregated_weight - 1 / (64 * 4 * 4) * torch.ones(1024, 64 * 4 * 4).float()) / (
                                                    local_epoch * 0.1)
            estimated_bias_gradient = -(
                        aggregated_bias + torch.from_numpy(bins).float().view(1024, 1).expand(1024, 64 * 16)) / (
                                                  local_epoch * 0.1)
            mse_fn = MSELoss()
            ...
            round_num, round_mse_score, round_PSNR_score = attack(estimated_weight_gradient.to(device),
                                                                  estimated_bias_gradient.to(device), original_data,
                                                                  l1output, batch_size, idx)
